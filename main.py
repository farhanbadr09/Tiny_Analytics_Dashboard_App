...
def preprocess(data):
    ...
    numeric_features, categorical_features = autoPrep.identify_numeric_categorical(df)
    relevant_columns = ['Response1', 'Response2']
    preprocessed_data = autoPrep.preprocess_data(df, numeric_features, categorical_features, relevant_columns=relevant_columns)

    processedData = preprocessed_data.to_csv(index=False)  # Use preprocessed_data here
